...
int main()
{
    ...
    while(t--){
        ...
        if(curr==0){
            for(ll i=0;i<=n;i+=2) vis[i]=1;
            if(n%2) curr++;
            for(ll i=curr;i<=n;i+=2) vis[i]=1;
        }
        else{
            for(ll i=curr;i<=n-curr;i+=2) vis[i]=1;
            if(n%2) curr++;
            for(ll i=curr;i<=n-curr+1;i+=2) vis[i]=1;
        }
        for(ll i=0;i<=n;i++) cout<<vis[i];
        cout<<nn;

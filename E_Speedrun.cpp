...
void dfs(ll vertex){
    ...
    for(ll child:g[vertex]){
        ...
        if(weight[vertex]<=weight[child]){
            tot+=(k-weight[child])+weight[vertex];
        }
        else{
            tot+=(weight[vertex]-weight[vertex]);
...
bool cmp(pll a,pll b){
    if(g[a.second].size()==g[b.second].size()) return a.first<b.first;
...
int main()
{
    ...
    while(t--){
        ...
        cin>>n>>m>>k;
        for(ll i=0;i<=n;i++){g[i].clear();vis[i]=0;weight[i]=-1;}

        vector<pll>vec(n);
        for(ll i=0;i<n;i++){
            cin>>vec[i].first;
            vec[i].second=i+1;
            weight[i+1]=vec[i].first;
        }
        sort(all(vec),cmp);
        // reverse(all(vec));
        ...
        lastmax=0;
        for(ll i=0;i<n;i++){
            cout<<i<<" "<<vec[i]<<" "<<g[vec[i].second].size()<<nn;
         if(!vis[vec[i].second]){
          
          ll curr=vec[i].second;
          dfs(curr);
          lastmax=vec[i].first;
         }
        }
